...
| Data layer (Phase 6) | **2026-03-27 architecture review:** No persistent database. Revenue-critical data (directory, shop, report templates, transactions) needs Cloudflare D1 + R2. Bootstrap + migration + repository scaffold are in-repo; remaining: create cloud resources and commit binding IDs in `wrangler.jsonc`. **2026-03-28:** Report routes + service wired (`POST /v1/reports`, `GET /v1/reports/:jobId`, `GET /v1/reports/:jobId/download`). Router upgraded to support path parameters. 88 tests passing. **2026-03-29:** D1 database `czy-main` created (APAC region), migration applied (6 tables live), binding wired in `wrangler.jsonc`. R2 bucket `czy-reports` created and bound. All cloud resources provisioned — Item J complete. **2026-04-05:** Schema migration added `price_cents`, `currency`, `description` columns to `report_templates`. First template `bazi-basic-en` seeded ($8.99, 10 sections). Repository column mismatch fixed. **2026-04-06:** Migrations applied to production D1 via new `migrate-d1.yml` workflow dispatch. Template live in production. See TODO items K + L for next steps. | 2026-03-27 | 2026-06-27 | D1 tables created and first API endpoint live |
