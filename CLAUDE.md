...
| Industrial-standard backend controls | Repository SQL boundary is CI-enforced (`scripts/check-service-sql-boundary.sh`, `npm run infra:boundaries:check`). OpenAPI contract (M) is now in-repo + CI-checked; remaining before paid-report scale: birth-data retention/deletion governance policy (N). | 2026-03-27 | 2026-05-27 | OpenAPI + governance SOP merged |
