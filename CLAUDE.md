...
| CI/CD Phase 5 | ~~**2026-03-27:** `deploy.yml` restructured into 4 jobs: test → build → deploy-pages + deploy-worker. Tests gate all deployments. Worker auto-deploys via `CLOUDFLARE_API_TOKEN` secret. Verify first workflow run succeeds after push.~~ **RESOLVED 2026-03-28:** First 5 consecutive 4-job workflow runs all succeeded. | 2026-03-27 | 2026-04-10 | ~~First successful 4-job workflow run confirmed~~ **DONE** |
| Data layer (Phase 6) | **2026-03-27 architecture review:** No persistent database. Revenue-critical data (directory, shop, report templates, transactions) needs Cloudflare D1 + R2. Bootstrap + migration + repository scaffold are in-repo; remaining: create cloud resources and commit binding IDs in `wrangler.jsonc`. **2026-03-28:** Report routes + service wired (`POST /v1/reports`, `GET /v1/reports/:jobId`, `GET /v1/reports/:jobId/download`). Router upgraded to support path parameters. 88 tests passing. **2026-03-29:** D1 database `czy-main` created (APAC region), migration applied (6 tables live), binding wired in `wrangler.jsonc`. R2 bucket `czy-reports` created and bound. All cloud resources provisioned — Item J complete. **2026-04-05:** Schema migration added `price_cents`, `currency`, `description` columns to `report_templates`. First template `bazi-basic-en` seeded ($8.99, 10 sections). Repository column mismatch fixed. **2026-04-06:** Migrations applied to production D1 via new `migrate-d1.yml` workflow dispatch. Template live in production. **2026-04-08:** PayPal checkout integration complete — `worker/services/paypal.js` (OAuth + Orders v2), `worker/routes/checkout.js` (3 endpoints), `worker/repositories/transaction-repository.js` rewritten. 113 tests passing. Remaining: PDF rendering layer, frontend PayPal Smart Buttons. | 2026-03-27 | 2026-06-27 | D1 tables created and first API endpoint live |
| GitHub Actions Node.js 20 deprecation | Actions `checkout@v4`, `setup-node@v4`, `configure-pages@v4`, `upload-artifact@v4`, `deploy-pages@v4` run on Node.js 20 which is deprecated. Forced to Node.js 24 on **2 June 2026**; removed **16 September 2026**. Upgrade to `@v5` tags or set `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`. | 2026-03-28 | 2026-06-01 | All actions updated to Node.js 24-compatible versions |
| Industrial-standard backend controls | Repository SQL boundary is CI-enforced (`scripts/check-service-sql-boundary.sh`, `npm run infra:boundaries:check`). OpenAPI contract (M) is CI-checked and birth-data governance policy (N) is documented in `docs/birth-data-governance-policy.md`. Remaining before paid-report scale: enforcement endpoints/jobs for scheduled deletion and audit-log wiring. | 2026-03-27 | 2026-05-27 | OpenAPI + governance SOP merged |
...
| D1 migrations | `.github/workflows/migrate-d1.yml` (manual dispatch) |
