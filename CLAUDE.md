...
| Data layer (Phase 6) | **2026-03-27 architecture review:** No persistent database. Revenue-critical data (directory, shop, report templates, transactions) needs Cloudflare D1. R2 needed for PDF reports. Bootstrap runbook/script added (`docs/d1-r2-bootstrap.md`, `scripts/bootstrap-d1-r2.sh`) but cloud resources still need creation + binding IDs in `wrangler.jsonc`. See `docs/architecture.md` Section 11 and `docs/architecture-redesign.md` Phase 6. Items J + K in TODO. | 2026-03-27 | 2026-06-27 | D1 tables created and first API endpoint live |
| Industrial-standard backend controls | **2026-03-27 onboarding audit:** Repository layer pattern, OpenAPI contract, and birth-data retention/deletion policy still required before paid-report scale. **2026-03-27 update:** D1 migration bootstrap added (`migrations/`, SQL baseline, CI migration check scaffold). See TODO items M/N/O. | 2026-03-27 | 2026-05-27 | D1 migrations + OpenAPI + governance SOP merged |
...
| Data layer (Phase 6) | **2026-03-27 architecture review:** No persistent database. Revenue-critical data (directory, shop, report templates, transactions) needs Cloudflare D1 + R2. **2026-03-27 updates:** bootstrap script/runbook and migration baseline are in-repo; repository scaffold is now added under `worker/repositories/*`. Remaining: create cloud resources and commit binding IDs in `wrangler.jsonc`. See TODO items J + K. | 2026-03-27 | 2026-06-27 | D1 tables created and first API endpoint live |
| Industrial-standard backend controls | **2026-03-27 onboarding audit:** Repository boundary now scaffolded and CI-enforced (`scripts/check-service-sql-boundary.sh`, `npm run infra:boundaries:check`). Remaining before paid-report scale: OpenAPI contract (M) and birth-data retention/deletion governance policy (N). | 2026-03-27 | 2026-05-27 | OpenAPI + governance SOP merged |
